...
watch(() => (page.props as any).errors, (newErrors) => {
  if (newErrors) {
          ...
          <CardContent class="space-y-4">
            <div class="space-y-2">
              ...
              <div class="relative">
                ...
                <Input id="nama" v-model="nama" placeholder="Masukkan nama lengkap" class="pl-9" />
              </div>
              <p v-if="errors.nama" class="text-destructive text-xs">{{ errors.nama }}</p>
            </div>
            ...
            <div class="space-y-2">
              ...
              <div class="relative">
                ...
                <Input id="email" v-model="email" type="email" placeholder="Masukkan email" class="pl-9" />
              </div>
            ...
            <div class="space-y-2">
              ...
              <div class="relative">
                ...
                <Input id="no_hp" v-model="no_hp" placeholder="Masukkan no handphone" class="pl-9" />
              </div>
              <p v-if="errors.no_hp" class="text-destructive text-xs">{{ errors.no_hp }}</p>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div class="flex flex-wrap gap-2">
              <button
                ...
                @click="pilihKursi(k)"
                :class="[
                  'flex h-10 w-10 items-center justify-center rounded-md text-sm font-medium transition-all cursor-pointer',
            ...
            <div class="mt-4 flex items-center gap-4 text-sm">
              <span class="flex items-center gap-1"><span class="bg-secondary inline-block size-4 rounded" /> Tersedia</span>
              <span class="flex items-center gap-1"><span class="bg-primary inline-block size-4 rounded" /> Dipilih</span>
              <span class="flex items-center gap-1"><span class="bg-muted inline-block size-4 rounded" /> Terisi</span>
            </div>
            <p v-if="errors.kursi" class="text-destructive mt-2 text-xs">{{ errors.kursi }}</p>
          </CardContent>
        </Card>
      </div>
        ...
        <Card class="sticky top-4">
          ...
          <CardContent class="space-y-3">
            <div class="flex items-center gap-2">
              ...
              <Badge variant="secondary" class="ml-auto">{{ jadwal.kereta.kelas }}</Badge>
            </div>
            <Separator />
            ...
            <div class="flex items-center gap-2 text-sm">
              ...
              <span>{{ jadwal.stasiun_tujuan.nama_stasiun }} ({{ jadwal.stasiun_tujuan.kota }})</span>
            </div>
            <Separator />
            <div class="flex items-center gap-2 text-sm">
              <Clock class="size-4" />
              <span>{{ formatWaktu(jadwal.waktu_berangkat) }} - {{ formatWaktu(jadwal.waktu_tiba) }}</span>
            </div>
            <Separator />
            <div class="flex items-center gap-2 text-sm">
              <ArmchairIcon class="size-4" />
              <span>Kursi: <strong>{{ kursi || '-' }}</strong></span>
            </div>
            <Separator />
            ...
            <Button class="mt-2 w-full cursor-pointer" size="lg" @click="handleSimpan">
